...
class MyCommon
{
    ...
    {
        if($group === 0){
            $data = DB::table("config")->where(['name' => $name, 'cancel_flag' => 0])->get()->toArray();
        }else{
            $data = DB::table("config")->where(['name' => $name, 'group' => $group, 'cancel_flag' => 0])->get()->toArray();
        }
        return $data;
    }
    ...
    {
        $data = DB::table('config')->where(['group' => $group, 'cancel_flag' => 0])->get()->toArray();
        return $data;
    }
     ...
     * @return false|int
     */
    public static function cur_day_last_time()
    {
        $time = time();
        $next_time = strtotime(date("Y-m-d 00:00:00")) + 86400;
